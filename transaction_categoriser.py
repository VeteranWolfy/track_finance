...
class TransactionCategoriser:
    def __init__(self, root):
        self.root = root
        self.root.title("Transaction Categoriser")
    ...
    def create_widgets(self):
        # Left frame - categorisation interface
        ...
        for key, value in self.categories.items():
            btn = tk.Button(self.category_frame, text=f"{key}: {value}",
                          command=lambda k=key: self.categorise_transaction(k))
        ...
        # Save button
        self.save_button = tk.Button(self.left_frame, text="Save Categorised Data",
                                   command=self.save_categorised_data)
    ...
    def handle_keypress(self, event):
        if event.char in self.categories:
            self.categorise_transaction(event.char)
    ...
    def save_categorised_data(self):
        if not self.categorised_data:
            messagebox.showwarning("Warning", "No categorised data to save!")
            return
        ...
        try:
            ...
            df = pd.DataFrame(self.categorised_data)
            ...
            self.existing_transactions.extend(self.categorised_data)
...
if __name__ == "__main__":
    root = tk.Tk()
    app = TransactionCategoriser(root)
    root.mainloop() 